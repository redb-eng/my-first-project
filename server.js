...
app.post('/api/predict', async (req, res) => {
  try {
    ...
    const referenceInfo = (req.body.referenceInfo || '').toString().trim();

    console.log('API キー:', process.env.VITE_ANTHROPIC_API_KEY ? '✅ 設定済み' : '❌ 未設定');
    console.log('画像枚数:', images.length);
    console.log('参考情報:', referenceInfo ? `あり (${referenceInfo.length}文字)` : 'なし');
...
{
  "raceName": "画像から読み取れるレース名（例: 芦ノ湖特別、有馬記念 など）。読み取れなければ空文字",
...
horsesは指数の高い順に最大5頭まで。notSelectedには、人気上位や注目されそうなのに今回は見送る馬を3〜5頭、理由付きで挙げること。bettingTicketsは2〜4点、期待値重視で具体的に。`;
    ...
    // ユーザーがExcel等で指定した「毎回考慮してほしい情報」をプロンプト冒頭に追加
    const finalPrompt = referenceInfo
      ? `【ユーザー指定の最重要参考情報】\n以下はユーザーが事前に用意した独自の予想ルール・データ・重視ポイントです。今回の予想で必ず尊重し、判断に反映させること：\n----------\n${referenceInfo}\n----------\n\n${promptText}`
      : promptText;
